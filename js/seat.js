...
// ─── PRICE ──────────────────────────────────────────────────────────────────
// Returns { total, byZone: { [zoneId]: pricePerSeat } }
async function calculatePriceDetailed() {
    if (selected.length === 0) return { total: 0, byZone: {} };
    ...
    // Fire one pricing request per zone with count=1 to get the per-seat price for that zone.
    // This way each zone's actual rate is respected, not averaged.
    const zoneIds = Object.keys(zonesCount);

    const perZonePrices = await Promise.all(
        zoneIds.map(async zid => {
            const payload = [{
                items: [{ zone_id: parseInt(zid, 10), count: 1 }],
                start_time: startISO,
                end_time: endISO,
            }];
            try {
                ...
                return { zid, pricePerSeat: res.data.total_amount };
            } catch (e) {
                console.error(`Pricing error for zone ${zid}:`, e);
                return { zid, pricePerSeat: 0 };
            }
        })
    );
    ...
    const byZone = {};
    let total = 0;
    perZonePrices.forEach(({ zid, pricePerSeat }) => {
        byZone[zid] = pricePerSeat;
        total += pricePerSeat * zonesCount[zid];
    });
...
async function updateUI() {
    ...
    if (selected.length > 0) {
        const { total, byZone } = await calculatePriceDetailed();
        window.currentTotalPrice = total;
        window.currentPriceByZone = byZone;

        const grouped = selected.reduce((acc, curr) => {
            const zn = curr.zoneName || zoneNameById[curr.zid] || 'Зона';
            acc[zn] = acc[zn] || { nums: [], zid: curr.zid };
            acc[zn].nums.push(`№${curr.n}`);
            return acc;
        }, {});

        label.innerHTML = Object.entries(grouped)
            .map(([z, { nums, zid }]) => {
                const pricePerSeat = byZone[zid] || 0;
                const count = nums.length;
                const zoneTotal = pricePerSeat * count;
                return `${z} <span>${nums.join(', ')}</span>`
                    + (pricePerSeat > 0
                        ? ` <span style="color:#aaa;font-size:11px;font-weight:500">(${pricePerSeat} ₸ × ${count} = ${zoneTotal} ₸)</span>`
                        : '');
            })
            .join('<br>') +
            `<br><span style="color:#6fcf97;font-size:15px;font-weight:700;">К оплате: ${total} ₸</span>`;
...
async function handleBooking() {
    ...
    try {
        const byZone = window.currentPriceByZone || {};

        // Each PC gets its own zone's per-seat price instead of the averaged total
        const data = selected.map(s => ({
            start_time: `${selectedDateStr}T${startTime}:00`,
            end_time: `${selectedEndDateStr}T${endTime}:00`,
            total_price: byZone[s.zid] || Math.round(window.currentTotalPrice / selected.length),
            computer_id: s.id,
            zone_id: s.zid,
            club_id: parseInt(clubId, 10),
        }));
